<script lang="ts">
	...
	$: totalPages = $page.data.pages;
	$: from = $page.data.page * $page.data.size;
	$: to = ($page.data.page + 1) * $page.data.size;
</script>
...
<footer class="flex items-center justify-between grid-cols-3 px-2 pb-4">
	<div class="w-20">
		{#if currentPage !== 0 && $page.data.count !== 0}
			<a
	...
	<div class="w-20">
		{#if currentPage !== totalPages - 1 && $page.data.count !== 0}
			<a
				href="/matches?page={currentPage + 1}"
				class="flex items-center justify-center px-2 py-2 text-sm uppercase border rounded border-gray-700/20 bg-gray-900/50"
			>
...
<section class="px-2">
	{#each $page.data.matches as match}
